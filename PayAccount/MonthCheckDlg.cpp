				...
				for (int j=0;j<nsize2;j++)//日期循环
				{
					...
					if (one2.isMember(CMD_RetType[EM_CMD_RETYPE_VALUE]))
					{
						...
						for (int k=0;k<nsize3;k++)//日做工循环
						{
							...
							{
								day_pay.strPayDay = one3[MPAYMSG[EM_GET_MPAY_PAYDAY]].asCString();
								day_pay.strDays = one3[MPAYMSG[EM_GET_MPAY_DAYS]].asCString();
							}
							else if(day_pay.type == DAYPAY_TYPE_JIJIAN)
							{
								day_pay.proID = one3[MPAYMSG[EM_GET_MPAY_PROID]].asInt();
								day_pay.strBookID = one3[MPAYMSG[EM_GET_MPAY_BOOKID]].asCString();
								day_pay.pay = one3[MPAYMSG[EM_GET_MPAY_PAY]].asCString();
...
{
	//设置回调
	g_SockClient.SetCallback(MonthCheckCallback,this);
